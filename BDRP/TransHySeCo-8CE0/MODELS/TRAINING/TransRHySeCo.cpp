...
void init() {
	...
	//---------------------------Initialize embeddings with transHySeCo results --------------
	//if(initPath!=""){	
		cout<<"Initialize embeddings with TransHySeCo\n";
		for (INT i = 0; i < relationTotal; i++)
			for (INT j = 0; j < dimensionR; j++)
				for (INT k = 0; k < dimension; k++)
					if (j == k)
		...
		FILE* f1 = fopen((inPath + "entity2vec"+note+".vec").c_str(),"r");
		for (INT i = 0; i < entityTotal; i++) {
			for (INT ii = 0; ii < dimension; ii++)
		...
		fclose(f1);
		FILE* f2 = fopen((inPath + "relation2vec"+note+".vec").c_str(),"r");
		for (INT i=0; i < relationTotal; i++) {
			for (INT ii=0; ii < dimension; ii++)
